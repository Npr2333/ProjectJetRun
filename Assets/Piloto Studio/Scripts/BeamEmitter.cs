...
namespace PilotoStudio
{
    ...
    public class BeamEmitter : MonoBehaviour
    {
            ...
            while (elapsedTime <= 1 + castTime)
            {
                ...
                else
                {
                    for (int i = 0; i < beams.Count; i++)
                    {
                        ...
                        beams[i].widthMultiplier = Mathf.Lerp(0, desiredWidth[i], elapsedTime / 1 + castTime);
                        elapsedTime += Time.deltaTime;
                    }
                    yield return null;
                }
            }
            if (elapsedTime > 1 + castTime)
            {
                for (int i = 0; i < beams.Count; i++)
                {
        ...
        private void PlayLineRenderers()
        {
            foreach (LineRenderer _line in beams)
            {
                beginOffset = transform.position - _line.transform.position;
                endOffset = transform.position - beamTarget.position;
